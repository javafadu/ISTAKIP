...
echo "<td align=\"center\"><form name=\"datasort4\" method=\"post\" action=\"durumlar.php\"><select name=\"status\">";
...
$sql = "SELECT date_format(last_change, '%Y-%m-%d') as last_change,deadline,open_date,taskid,priority,title,description,statusname,catname,status,display,personnel,ofiskod FROM $taskstable WHERE display = 'Y' AND statusname = '$status2'  ORDER BY deadline ASC";
$result = mysql_query($sql);
}
